# Stage 11 — Multi-Agent Systems, Handoffs & A2A Interoperability

## Why this stage exists

Tiny-Agent deliberately waits until Stage 11 before teaching multi-Agent systems.
...
The central Stage 11 question is:
...
By the end of Stage 11 you should be able to explain and implement:
...
Budgets and handoff history belong to one logical run, just like Stage 09's `GuardedRunState`.
...
Tiny-Agent constructs current A2A 1.0 Agent Card and Message objects offline. A complete network server/client deployment is intentionally deferred to Stage 13, where service identity, HTTP serving, task storage and deployment can be taught honestly.
...
Read/run Stage 11 in this order:
...
For Stage 11 framework/interoperability comparisons:

```bash
python -m pip install -e ".[dev,stage11]"
```

Stage 11 optional dependencies target:
...
Run the Stage 11 tests directly while studying the stage; repository-maintenance automation is intentionally outside the curriculum tree.
...
Stage 03 already teaches state graphs. Revisit subgraphs/supervisor patterns after you understand the Stage 11 mechanisms:
...
Those belong with Stage 13 deployment and infrastructure.

Stage 11 establishes the architecture and interoperability semantics first.
