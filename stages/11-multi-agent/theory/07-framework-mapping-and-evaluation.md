...
Stage 11 deliberately implements coordination mechanisms before introducing a multi-Agent runtime framework.
...
Stage 11 should never benchmark multi-Agent only against itself.
...
These metrics fit naturally into Stage 10 `RunArtifact.metrics`.
...
A Stage 11 trajectory may look like:
...
Stage 10 trajectory-evaluation concepts apply directly.
...
But raw hidden prompts/private context should still follow Stage 10 capture policy.
...
Stage 11 does not claim to solve:
...
These need Stage 13 deployment infrastructure.
