...
三个 worker 各花 `$0.02`，可能 latency 下降但成本变成 3 倍。是否值得，应由 Stage 10 metrics 回答。
...
- retry specialist：必须满足 Stage 09 retry-safe 规则；
...
Stage 10 trace 可以发现并判断这个成本是否换来了有价值 diversity。
...
Stage 11 不声称已经解决 full distributed cancellation，留到 Stage 13 infrastructure。
