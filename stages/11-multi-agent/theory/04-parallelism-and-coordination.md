...
Stage 10 metrics should decide whether that trade is acceptable.
...
Only if the failure and operation are retry-safe, following Stage 09 rules.
...
Tool loops from Stage 09 become Agent loops:
...
Tiny-Agent Stage 11 begins with call/handoff/repeated-edge limits.
...
Stage 10 lets you detect repeated work and decide whether diversity justified the cost.
...
Stage 11 does not claim full distributed cancellation. That belongs with Stage 13 service deployment and task infrastructure.
...
Useful Stage 11 metrics include:
