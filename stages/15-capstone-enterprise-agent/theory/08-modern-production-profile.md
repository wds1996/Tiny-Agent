...
The Stage 15 offline Agent is intentionally deterministic and inspectable. A real deployment should **compose stronger infrastructure behind stable domain boundaries** rather than rewrite the Agent every time a backend changes.
...
Do not create infinite "critic until perfect" loops. Stage 09 budgets still apply.
