...
The final architecture is useful only if callers can actually reach it. Stage 15 therefore exposes the same domain Agent through multiple boundaries without duplicating the research logic.
...
## Stage 13 service constraints still apply
...
The Stage 15 Dockerfile packages the application and starts the FastAPI example. It is deliberately a baseline, not a claim of enterprise deployment completeness.
