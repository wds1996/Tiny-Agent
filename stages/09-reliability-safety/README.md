# Stage 09 — Reliability, Safety & Tool Governance

Stage 09 turns Tiny-Agent from a system that **can execute** into one that can **refuse, bound, retry, time out, and mediate execution predictably**.
...
Stage 09 asks the control-plane question:
...
Stage 06 is particularly important because Stage 09 builds on:
...
Stage 09 begins by correcting two production gaps in the existing integrated runtime.
...
Stage 09 policy code must inspect a Tool's schema/handler before execution without reaching into a private `_tools` dictionary.
...
After Stage 09, you should be able to:
...
```bash
python stages/09-reliability-safety/code/error_model.py
```
...
Token/cost values are recorded when provider usage metadata is available; Stage 10 will make those values observable and evaluable.
...
The Stage 01 `AgentRuntime` remains intentionally small. Stage 09 adds a stronger execution layer around `ToolRegistry` instead of turning the beginner runtime into a 500-line security framework.
...
For the mature Stage 09 comparison libraries and all tests/examples:
...
Then reread the Stage 09 warning that timing out a worker thread is not equivalent to killing the underlying synchronous function.
...
2. Stage 09 Theory 01 + error_model.py
...
Stage 09 establishes a **guarded runtime architecture**, not a claim of complete enterprise security.
...
- red-team automation;
- tracing/metrics/evaluation dashboards (Stage 10);
- service deployment and infrastructure hardening (Stage 13).
...
By the end of Stage 09, you should be able to build and explain:
