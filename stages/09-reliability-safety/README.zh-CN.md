# Stage 09 — Reliability、Safety 与 Tool Governance

> Language: [English](README.md) | 简体中文

Stage 09 要把 Tiny-Agent 从一个“**能够执行**”的系统，升级成一个能够**明确拒绝、限制资源、超时、重试，并以可预测方式治理执行**的系统。
...
Stage 09 开始追问 control plane 层面真正关键的问题：
...
Stage 06 尤其重要，因为 Stage 09 直接建立在下面三个结论上：
...
Stage 09 的 policy code 需要在 execution 前检查 Tool schema/handler，而不应该直接访问私有 `_tools` 字典。
...
当 provider usage metadata 可用时记录 token/cost；Stage 10 会继续把这些变成可观察、可评估的数据。
...
Stage 01 `AgentRuntime` 仍然保持小而透明。Stage 09 是在 `ToolRegistry` 外面增加更强的 execution layer，而不是把初学者 runtime 改造成 500 行安全框架。
...
安装成熟 Stage 09 对比库以及完整 tests/examples：
...
Stage 09 建立的是 **guarded runtime architecture**，不是“已经完成全部企业安全”的声明。
