...
Stage 09 begins with a boring fact that saves exciting outages:
...
Stage 09 changes the reusable runtime boundary:
...
Because Stage 10 observability will want to answer:
...
Stage 09 deliberately avoids guessing that every `TypeError` came from a bad ToolCall.
...
6. Why failures need structured codes before Stage 10 observability.
