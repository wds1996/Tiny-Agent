...
Stage 09 adds a rule for composing them safely:
...
# 5. Why not put every Stage 09 feature into `ToolRegistry`?
...
Stage 09 adds orchestration/policy around it through:
...
The legacy runtime still intentionally does not become the full Stage 09 executor.
...
# 7. Audit events belong nearby, but Stage 10 owns observability

Stage 09 already has structured information such as:
...
But Stage 09 does not build a full tracing system.

Stage 10 will answer:
...
# 8. What Stage 09 still does not solve
...
What Stage 09 does provide is the correct architecture for plugging those systems in.
...
15. What event will Stage 10 log/evaluate?
```
...
Stage 09 replaces the motivational speech with actual controls.
...
4. How Stage 09 feeds structured events into Stage 10.
