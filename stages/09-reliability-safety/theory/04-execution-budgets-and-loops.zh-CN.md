...
所以 Stage 09 ledger 在 provider metadata 可用时记录：
...
Stage 10 会把这些数据纳入 observability。
...
Stage 09 对 exact call 做 fingerprint：
...
所以 Stage 09 把 budget 当作 runtime policy，而不只是 cost optimization。
