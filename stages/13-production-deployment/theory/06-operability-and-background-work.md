...
Stage 10 provides evaluation/tracing. Stage 13 adds service saturation and infrastructure dimensions.
...
If shutdown kills a process in the middle of a side effect, retries must respect Stage 09 idempotency semantics.
...
Tiny-Agent's teaching `SQLiteRunQueue` uses a simpler subset, while Stage 14's `TaskLedger` models sub-work inside a run.
