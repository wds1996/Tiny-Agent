...
Stage 10 负责 evaluation/tracing；Stage 13 补 service saturation 与 infra dimension。
...
如果 shutdown 正好发生在 side effect 中间，retry 仍必须遵循 Stage 09 idempotency semantics。
...
Tiny-Agent `SQLiteRunQueue` 使用更小 subset；Stage 14 `TaskLedger` 管一个 run 内部 sub-work。
