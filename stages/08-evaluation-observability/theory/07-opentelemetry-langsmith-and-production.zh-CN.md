...
优势：vendor-neutral instrumentation、cross-service context、existing exporters/backends、ecosystem integration、semantic conventions，以及未来 Stage 10 service tracing。
...
所以 Stage 08 不新增 `Span.add_event()` / `Span.record_exception()` instrumentation。

Operation 保持 span；event-like record 在 Stage 10 扩展 telemetry 时使用 correlated log。
...
## 13. Observability 不能破坏 Stage 07 Security
