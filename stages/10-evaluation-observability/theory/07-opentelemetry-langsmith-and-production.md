...
After building the local mechanism, Stage 10 maps it to two mainstream tools that solve different layers.
...
- easier Stage 13 service-to-service tracing.
...
Tiny-Agent's Stage 10 OpenTelemetry adapter therefore does not introduce new `Span.add_event()` or `Span.record_exception()` instrumentation.

Operations remain spans. Event-like records should be modeled as correlated logs when Stage 13 expands telemetry.
...
Its current concepts map naturally to Stage 10:
...
# 13. Observability must respect Stage 09 security
