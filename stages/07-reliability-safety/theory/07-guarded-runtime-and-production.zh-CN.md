...
## 5. 为什么不把 Stage 07 全塞进 `ToolRegistry`？
...
但 legacy runtime 故意没有升级成完整 Stage 07 executor。
...
## 7. Audit Event 已经有了语义，但 Stage 08 才负责 Observability
...
但 Stage 07 不负责构建完整 tracing system。
...
Stage 07 提供的是：让这些系统未来有正确位置接入的 architecture。
...
Stage 07 做的事情，就是把这封“鼓励信”换成真正可执行的权限、预算、审批和隔离控制。
...
4. Stage 07 如何为 Stage 08 提供 structured event；
