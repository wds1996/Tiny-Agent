...
Stage 07 把这个问题当成 architecture problem，而不是一句 prompt 技巧。
...
Stage 07 把它落实到 execution governance。
...
Stage 07 故意放了一个很小的 detector，识别例如：
...
Stage 07 example 用 child process，因为 parent 能在 deadline 后 terminate 它。
