...
Stage 09 先手写 coordination mechanism，再引入 multi-Agent framework。目的不是拒绝 framework，而是先知道它自动化了什么、哪些 application policy 仍属于你。
...
Stage 08 trajectory concepts 直接适用。
...
可记录 source/target/coordination.mode，但 raw hidden prompt/private context 继续受 Stage 08 capture policy 控制。
...
不宣称解决 distributed registry、enterprise service identity、cross-service transaction、durable queues、remote cancellation、A2A task DB、multi-region routing、service mesh policy、full distributed tracing。这些进 Stage 10。
