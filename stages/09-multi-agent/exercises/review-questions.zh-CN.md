...
53. 增加 worker timeout，但不要复制 Stage 07 retry policy；思考 timeout 应属于哪个层。
...
57. 用 Stage 08 tracer protocol 给 `delegate()` / `handoff()` 加 span，默认不 capture raw delegation task text。
